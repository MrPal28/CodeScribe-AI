...
app = Flask(__name__)
genai.configure(api_key="AIzaSyCLHCuTA_2Q4pYY-7DvBbfeaq_nIzMFetE")

# Modified prompt (no alternative suggestions requested)
default_prompt = (
...
def chat():
    ...
    try:
        ...
        if is_inappropriate:
            # Extract words and explanations (fallback regex, works best with structured Gemini response)
            matches = re.findall(r"(?i)(?:word)[:\-]?\s*['\"]?([\w\-]+)['\"]?\s*[\n\-]*.*?explanation[:\-]?\s*(.*?)(?:\n|$)", result_text)
